...
exports.buildCraneModel = void 0;
const THREE = require("three");
function buildCraneModel(crane) {
    ...
    const base = new THREE.Mesh(new THREE.BoxGeometry(2, base_height_m, 2), new THREE.MeshBasicMaterial({ color: '#002776' }));
    base.translateY(base_height_m / 2);
    const lift = new THREE.Mesh(new THREE.BoxGeometry(lift_width_m, lift_height_m, 1), new THREE.MeshBasicMaterial({ color: '#009c3b' }));
    lift.name = 'lift';
    base.add(lift);
    lift.translateY((lift_height_m) / 2 + (base_height_m / 2));
    const rail = new THREE.Mesh(new THREE.BoxGeometry((rail_length_m), rail_height_m, 1), new THREE.MeshBasicMaterial({ color: '#ffdf00' }));
    rail.name = 'rail';
    lift.add(rail);
    rail.translateX(((rail_length_m) / 2) + (lift_width_m / 2));
    rail.translateY((lift_height_m / 2) - (rail_height_m / 2) - gripper_height_m);
    const gripper = new THREE.Mesh(new THREE.BoxGeometry(gripper_length_m, gripper_height_m, 1), new THREE.MeshBasicMaterial({ color: 'red' }));
...
exports.buildCraneModel = buildCraneModel;
//# sourceMappingURL=crane_model.js.map